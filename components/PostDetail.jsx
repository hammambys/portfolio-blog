...
const PostDetail = ({ post }) => {
  const getContentFragment = (index, text, obj, type) => {
    ...
    switch (type) {
      ...
      case "paragraph":
        return (
          <p key={index} className="mb-8 font-normal">
            {modifiedText.map((item, i) => (
              <React.Fragment key={i}>{item}</React.Fragment>
            ))}
          </p>
          ...
          <div className="flex justify-center mb-4" key={index}>
            <div>
              <img
                ...
                src={obj.src}
              />
              <p className="text-center mt-2">{obj.altText}</p>
            </div>
          </div>
        );
      default:
  ...
  return (
    <>
      <div className="bg-white border shadow-lg rounded-lg lg:p-8 pb-12 mb-8 dark:text-sand_dollar">
